...
class LlamadaFuncion(NodoAST):
    ...
    def interpretar(self, arbol, table):
        x = table.retornarSimbolo(self.nombre.lower())#Retorna el simbolo donde guardo el objeto Funcion
            ...
            if isinstance(x.getValor(), Funcion):#VER SI EL GETVALOR ES UNA FUNCION 
                ...
                if funcion.getParametros() == None:
                    ...
                    if self.parametros == None:#si entra aca es por que no trae parametros y eso no lo acepta
                        ...
                        nuevoentorno = TablaSimbolos(table)#creamos un nuevo entorno, aquí se guardaran las variables 
                        ...
                        if len(listaparametros) == len (listavalores):
                            ...
                            for i in listaparametros:
                                ...
                                if isinstance(matriz, Arreglo):
                                    nuevosimbolo = Simbolo(i.getIdentificador(), matriz, "arreglo", self.fila, self.columna)
                                    table.addSimbolo(nuevosimbolo)
